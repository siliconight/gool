...
- **Tank delays 1 and 2 had reversed read/write ordering since
  v0.29.0 ship.** The Process loop did:
  ...
  Since `Read()` returns `buf[pos]` and `Write(x)` writes the same
  `buf[pos]`, the Read after Write returned the value just written
  — **a zero-sample delay**. The Allpass class above uses the
  correct order (Read → Write → Advance, see `Allpass::Step`); the
  tank's two long delay lines per half didn't, so Dattorro's
  4453 / 3720 / 4217 / 3163-sample tank delays (~150 / 125 / 140 /
  105 ms regardless of SR) were short-circuited.

  Those long delays are *the* literal foundation of a Dattorro
  plate's diffuse late-field smoothness — when they don't function,
  the reverb tail is generated by the modAP + ap allpasses +
  cross-feedback alone, which is exactly the topology you'd expect
  to sound metallic and harsh (short recirculations, no large-scale
  smearing). v0.57.0 reverses the order to Read → Write → Advance
  on both delay1 and delay2 in both tank halves.

  This is THE root cause of the pre-v0.57.0 "metallic ringing"
  reverb character. v0.56.0's modDepth-scaling fix and v0.56.0's
  diffusion-default bump were both real bugs and real improvements,
  but neither could compensate for the tank delays simply not
  functioning. With v0.57.0 the tank produces the diffuse late
  field Dattorro's paper designed for.

  Why this wasn't caught earlier: the system still produced a
  reverb-shaped tail (cross-feedback alone gives a decay), and the
  output taps (which use `ReadTap(offsetBack)`, not the immediate
  `Read()`) returned non-zero values because they don't depend on
  the immediate Read-after-Write ordering. So it sounded plausible
  enough to pass smoke tests, just consistently harsh.

### Changed

- **`hfDamping` default raised 0.3 → 0.45.** At the previous 0.3
  default, the one-pole HF damping shelf had cutoff near 9.6 kHz
  at 48 kHz SR — essentially transparent, no per-circulation HF
  rolloff. Real acoustic spaces absorb HF faster than LF on every
  reflection; 0.45 puts the per-pass cutoff at ~6.4 kHz (~4.5 kHz
  effective after one full figure-8 lap), in the natural-room
  range. Combined with the now-functional tank delays, this gives
  the tail a recognisable physical-space character instead of a
  flat synthetic brightness. Existing configs that explicitly set
  `hf_damping` are unchanged; bright synthetic tails still set
  0.0–0.2 explicitly; dark / heavily damped spaces still set 0.6+.

### Added

- **`docs/audio_design/reverb_dattorro.md` audit section.** Auditing
  gool's reverb against the canonical reverb-design literature
  framework (echo density, in-loop damping, modulation, feedback
  matrix, separate ERs, nonlinear aliasing, ping killing,
  full-bandwidth preservation). Documents what gool does well,
  what v0.57.0 fixed, what's deferred (tank delay modulation
  v0.58.0; FDN-matrix `ReverbHall` as a separate effect; dedicated
  early-reflection stage), and the references used. References
  the academic public-domain sources (Dattorro 1997, Smith PASP,
  DAFX, Pirkle) — not the commercial signature units.

### Notes

- The user pointed to a checklist of reverb-quality factors and
  said "study what they did and make it gools" — i.e. learn from
  the public design literature, build gool's faithful version
  rather than chase any specific commercial sound. That framing
  is exactly right: gool's plate is a faithful Dattorro
  implementation; v0.57.0 is about making it actually behave the
  way Dattorro's paper describes, which it hadn't been.
- I cannot hear the result from inside this container. The
  Read/Write ordering bug is provable by code reading (which the
  changelog and design-doc audit walk through); the perceptual
  result is going to be a substantially less metallic, longer,
  smoother tail. If after listening the tail is now TOO long, the
  fix is to reduce the `decay` parameter on existing configs —
  the previous configs were tuned to compensate for the broken
  short tank, so they'll have decay values that are too high for
  the now-functional tank.
- Existing configs may need re-tuning. Specifically: `decay`
  values set to compensate for the previous short tank will now
  produce a longer tail than intended. Halving `decay` is a
  reasonable starting point if a preset sounds too washy. The
  shipping presets in `gool_presets.gd` were tuned against the
  broken behavior and may need a follow-up tuning pass — flagged
  as work for v0.57.1 once the perceptual result is auditioned.

### Backward compatibility

- No API changes. No config schema changes. No new parameters.
- Audio behavior changes (intentional and substantial): every
  reverb instance sounds different post-fix. Existing configs
  that explicitly set `decay` will produce longer, smoother tails
  than before. Existing configs that explicitly set `hf_damping`
  are unchanged; the default-value bump only affects configs
  that omitted the parameter or used the autoload default.
...
`docs/audio_design/saturation_v2.md`. This release ships only
the highest-leverage piece: **Antiderivative Anti-Aliasing
(ADAA)** wrapped around the existing tanh waveshaper. Eliminates
audible foldover aliasing at drive ≥ 2.5 on bright/transient
sources with no API change and no migration concern.

### What the doc covers (committed at `docs/audio_design/saturation_v2.md`)

Four phases targeting v0.38.0 through v0.41.0:
- **Phase 1 (this release, v0.38.0)** — ADAA on existing tanh
- **Phase 2 (planned v0.39.0)** — Mode selector + 4 distinct
  shapes (Tanh / Tube / Tape / Diode)
- **Phase 3 (planned v0.40.0)** — Auto-compensation + DC blocker
  + param smoothing
- **Phase 4 (planned v0.41.0)** — Tone filter (pre/de-emphasis
  tilt)

Each phase is self-contained and ships value independently.
Phase 1 is the biggest acoustic win.

### Why this matters acoustically

The pre-v0.38.0 implementation used trivial `tanh((x+bias)*drive)`
with no oversampling and no antialiasing. The header acknowledged
this and recommended "do aggressive distortion offline." At
typical drive (≤ 2.5) on dry sources this was inaudible; at
drive ≥ 2.5 on bright/transient material (gunshot clicks,
cymbals, fricatives in voice, environment debris) the upper
harmonics generated by tanh exceeded Nyquist and folded back as
in-band aliasing, audible as "graininess" or "grit" sitting on
top of the intended saturation character.

The textbook fix is 2× polyphase oversampling around the shaper.
ADAA is the modern alternative (Parker, Zavalishin, Le Bivic,
DAFX 2016): cheaper, no buffer allocations, no polyphase filter
design, comparable aliasing reduction. The right tool for our
cost budget.

### How it works

For a memoryless nonlinearity `f(x)` with first antiderivative
...
y[n]  =  f((x[n] + x[n-1]) / 2)      otherwise (midpoint fallback)
```

For tanh, `F(x) = log(cosh(x))`. We use the numerically stable
form `|x| + log1p(exp(-2|x|)) - log(2)` to avoid overflow at
large `|x|` (direct `log(cosh(x))` overflows around |x|≈700).
The ε threshold is 1e-6 with double-precision internal compute;
the per-sample arithmetic promotes float input to double for the
ADAA math and converts back at the boundary.

Measured aliasing reduction on a 19 kHz sine at 48 kHz SR at
drive=3.0: the 9th harmonic of 19 kHz (171 kHz) folds back to
21 kHz under trivial tanh; under ADAA it's suppressed by ~50 dB.
The new `TestADAASuppressesAliasing` test asserts a conservative
-40 dB threshold (a factor of 100 between fundamental and worst
alias-band bin) to leave headroom for FP variation across
platforms.

### Tradeoffs

**Half-sample group delay.** The integral-mean form of ADAA
shifts the output by half a sample relative to the trivial
shaper. Inaudible (≤ 10 µs at 48 kHz, well below the smallest
audible time difference). The wet path is mixed against the dry
path which would need the same delay to null bit-exactly, but
nulling isn't a use case here.

**Output at drive ≈ 1.0 is no longer bit-identical to plain
tanh.** Within ~1e-4 absolute on typical input (below float
quantization on most signals). The existing
`TestUnityDriveMatchesTanh` test was updated to check
steady-state output (sample 32+) rather than the cold-start
sample, since ADAA's first sample on a constant input uses the
cold-start state `x[n-1]=0` and computes an integral-mean rather
than the steady-state tanh value. From sample 2 onward, on
constant input, the midpoint fallback fires and output matches
trivial tanh exactly.

**Cost: ~3× per-sample.** The `log_cosh` evaluation is the
dominant cost (two transcendentals: `log1p` and `exp`). On a
typical buffer, total saturation effect cost is still <1% of a
modern core at 48 kHz stereo. The `mix == 0` early-out path is
preserved bit-exactly — buses with the effect installed but
turned off pay exactly nothing.

### No API change

All four existing parameters (drive, mix, output_gain, bias)
keep identical semantics. JSON configs from v0.37.0 and earlier
load and run with no migration. Existing presets sound nearly
identical at drive ≤ 2.0; the audible difference is the
suppression of high-drive aliasing on bright sources, exactly
what we wanted.

The `SaturationEffect` class grew one private member:
`std::vector<double> prevDriven_` holding per-channel `x[n-1]`
state. Allocated and zero-initialized in `Prepare()`; the
defensive resize in `Process()` covers unit tests that bypass
`Prepare` or hot-plug channel counts.

### Test coverage

`tests/unit/saturation_test.cpp`:
- All seven existing tests preserved, six unchanged in intent
- `TestUnityDriveMatchesTanh`, `TestBiasDoesNotIntroduceDc`,
  `TestMixInterpolatesLinearly`, `TestRuntimeParameterChanges`:
  now check steady-state (sample 32) rather than sample 0,
  since ADAA's cold-start transient differs from steady-state
- `TestBypassIsIdentity`, `TestDriveCompressesPeaks`,
  `TestSymmetryWithoutBias`: pass unchanged (the bypass
  path is preserved bit-exactly; peak measurements are
  insensitive to the cold-start sample; symmetry holds at the
  cold-start sample too because `log(cosh)` is even)
- NEW: `TestADAASuppressesAliasing` — DFT-based aliasing test
  on a 19 kHz sine at drive=3.0, asserts alias-band energy is
  at least 40 dB below the fundamental. This is the test that
  proves the ADAA code does what the design doc says it does.

`tests/unit/saturation_profile_test.cpp`:
- One assertion updated (`maxAbs < 1e-6f` on zero-input through
  a biased profile): now measured over steady-state frames
  (skip the cold-start sample) for the same reason as above.
- All other profile assertions unchanged.

### Touch summary

- `src/audio_engine/dsp/saturation_effect.h` — added
  `prevDriven_` state vector and `<vector>` include; rewrote
  topology comment to describe ADAA
- `src/audio_engine/dsp/saturation_effect.cpp` — new
  `log_cosh()` helper, new `adaa1_tanh()` helper, replaced the
  inner Process loop with the ADAA arithmetic, double-precision
  state and intermediate computation
- `tests/unit/saturation_test.cpp` — updated 4 existing tests to
  steady-state checks, added `TestADAASuppressesAliasing` with
  a hand-rolled DFT-bin helper, removed orphan `Mean()` helper
- `tests/unit/saturation_profile_test.cpp` — one steady-state
  fix on the zero-input-through-biased-profile assertion
- `docs/audio_design/saturation_v2.md` — committed the design
  doc (was previously a standalone artifact, now lives next to
  `reverb_dattorro.md` for discoverability)
