// audio_engine/decoders/ogg_vorbis_decoder.h
//
// IAudioDecoder over Ogg Vorbis files via stb_vorbis. Compiled when
// AUDIO_ENGINE_DECODERS_OGG is defined.

#ifndef AUDIO_ENGINE_DECODERS_OGG_VORBIS_DECODER_H
#define AUDIO_ENGINE_DECODERS_OGG_VORBIS_DECODER_H
...
public:
    static std::unique_ptr<OggVorbisDecoder> CreateFromFile(const char* path);
    static std::unique_ptr<OggVorbisDecoder> CreateFromMemory(const uint8_t* data,
                                                                uint64_t       size);
...
#endif // AUDIO_ENGINE_DECODERS_OGG_VORBIS_DECODER_H
