...
void ReverbEffect::Process(float* output, uint32_t frames, uint32_t channels,
    ...
    for (uint32_t f = 0; f < frames; ++f) {
        ...
        // Tank half A.
        // v0.57.0: read BEFORE write on delay1/delay2. The previous
        // order was Write→Read→Advance, which returned the value
        // we'd just written and collapsed these delays to zero
        // length. The Allpass class above does this correctly
        // (Read→Write→Advance) but the tank delays didn't, so
        // Dattorro's long tank delays (4453/3720/4217/3163 samples,
        // ~150ms / 125ms / 140ms / 105ms at any SR) were
        // short-circuited — the *literal foundation* of the plate's
        // diffuse late-field smoothness was off. Tail was being
        // generated by modAP + ap + cross-feedback alone, which is
        // why it sounded so metallic/short despite the modulation
        // and damping being correctly implemented. THE primary
        // root cause of pre-v0.57.0 reverb harshness.
        TankHalf& A = tank_[0];
        const float aIn  = tankIn + crossFromB_;
        const float aMod = A.modAP.Step(aIn);
        const float aD1 = A.delay1.Read();        // read L-sample-old first
        A.delay1.Write(aMod);                      // overwrite at pos
        A.delay1.Advance();
        const float aDamped = A.damping.Step(aD1);
        const float aApOut  = A.ap.Step(aDamped);
        const float aOut = A.delay2.Read();       // same fix on delay2
